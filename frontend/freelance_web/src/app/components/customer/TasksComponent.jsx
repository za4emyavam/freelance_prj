...
import {retrieveAllCustomerTasks} from "../../api/CustomerApi";
...
export default function TasksComponent() {
    ...
    function refreshTasks() {
        setIsLoading(true)
        Promise.all([
            retrieveAllCustomerTasks(orderBy, order, offset),
        ]).then((responses) => {
    ...
    const handleOrderBy = (value) => {
        ...
        retrieveData(value, newOrder, offset)
    }

    const handlePageChange = (newOffset) => {
        setOffset(newOffset)
        retrieveData(orderBy, order, newOffset)
    }

    function retrieveData(orderBy, ordering, offset) {
        setIsLoading(true)
        Promise.all([
            retrieveAllCustomerTasks(orderBy, order, offset),
        ]).then((responses) => {
    ...
    const handleTaskClick = (param) => {
        navigator(`/customer/tasks/${param}`)
    }
            ...
            <div className="overflow-x-auto sm:rounded-lg w-3/4">
                ...
                <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4"
                    ...
                    <span
                        ...
                        className="font-semibold text-gray-900">{offset < 1 ? (totalCount < 5 ? `1-${totalCount}` : "1-5") : (`${offset - 4}-${offset}`)}</span> of <span
